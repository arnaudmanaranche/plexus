...
const { createFsContext, buildGraph, attachDiskSizes } = require('./graph');
...
async function runFilesystemAnalysis(opts) {
  ...
  if (htmlMode) {
    await attachDiskSizes(graph, ctx);
    ...
    if (bundleSizeMode) {
      ...
      for (let i = 0; i < directKeys.length; i += limit) {
        const batch = directKeys.slice(i, i + limit);
        await Promise.all(
          batch.map(async pkg => {
            ...
            if (result) {
              ...
              console.log(`  ${color(pkg, 'cyan')} … ${formatBytes(result.size)} (gzip: ${formatBytes(result.gzip)})`);
            } else {
              console.log(`  ${color(pkg, 'cyan')} … ${color('n/a', 'gray')}`);
            }
          }),
        );
      }
